...
export class AuditLog {
    ...
    async append(kind, skill, period, payload = {}) {
        const entry = {
    ...
    async replayChain() {
        ...
        for (const line of lines) {
            ...
            if (e.prevHash !== expected) {
                throw new Error(`AuditLog: chain broken at ${e.id} — expected prevHash ${expected}, got ${e.prevHash}`);
            }
            const recomputed = hashEntry(e);
            if (recomputed !== e.hash) {
                throw new Error(`AuditLog: tampered entry ${e.id} — recomputed hash ${recomputed} ≠ stored ${e.hash}`);
            }
            out.push(e);
            expected = e.hash;
        }
...
function nowUtcMicroseconds() {
    // hrtime gives nanosecond resolution; we surface microseconds in ISO format.
    const now = new Date();
    const usSinceMs = Number(process.hrtime.bigint() % 1000000n);
