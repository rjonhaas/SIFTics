...
import { SITUATION_UNIT_ID } from "./routing.js";
...
export class OperationalPeriodLoop {
    ...
    async run(directive, dryRun) {
        ...
        await this.deps.auditLog.append(closeReason === "budget" ? "incident_truncated" : "incident_close", "command-staff/incident-commander", periods.length, { periods_run: periods.length, close_reason: closeReason });
        const finalCop = await this.deps.copStore.readCurrent("planning/situation-unit");
        return { periods, finalCop, closeReason };
    }
    async runPeriod(p, directive, dryRun) {
        ...
        if (!dryRun && this.deps.runner !== null) {
            ...
            throw new Error("Live swarm execution not yet implemented at this milestone. " +
        ...
        const cop = {
            ...
            contradictions: [],
            narrative: `[dry-run period ${p}] selective activation chose ${activeBranches.length} branch(es); ` +
    ...
    selectBranches(directive) {
        const sources = directive.evidence_sources;
        const has = (t) => sources.some((s) => s.type === t);
        const active = new Set(["operations/triage-branch"]);
        if (has("memory"))
            active.add("operations/memory-branch");
        if (has("pcap"))
            active.add("operations/network-branch");
        if (has("disk") || has("logs") || has("velociraptor"))
            active.add("operations/forensics-branch");
    ...
    formatIcReasoning(directive, active, standDown) {
...
//# sourceMappingURL=operational_period.js.map