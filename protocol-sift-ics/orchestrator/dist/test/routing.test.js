...
test("broker submission honors per-skill allowlist", async () => {
    const router = await buildRouter();
    const allowed = router.authorizeBrokerSubmission("operations/forensics-branch", "extract_mft_timeline");
    assert.equal(allowed.allow, true);
    const denied = router.authorizeBrokerSubmission("operations/forensics-branch", "vol_pslist" // Memory Branch tool, not on Forensics allowlist
    );
...
test("IC may delegate to all command-staff via wildcard, but not to a branch directly", async () => {
    const router = await buildRouter();
    assert.equal(router.authorizeDelegation("command-staff/incident-commander", "command-staff/safety-officer").allow, true, "IC → safety-officer via command-staff/* glob");
    ...
    const direct = router.authorizeDelegation("command-staff/incident-commander", "operations/forensics-branch");
