...
export interface ToolRequest {
  /** Identity of the requesting subagent — set by the orchestrator from
   *  subagent context, never trusted from the payload. */
  callerId: string;
  /** MCP tool the caller wants invoked. */
  toolName: string;
  /** Tool arguments — passed through to the MCP server. */
  arguments: Record<string, unknown>;
  /** Originating skill if the broker is dispatching on someone else's
   *  behalf. The broker's own caller identity stays in callerId. */
  onBehalfOf?: string;
}
...
export class Router {
  ...
  authorizeMcpInvocation(request: ToolRequest): RoutingDecision {
    if (request.callerId !== TOOL_BROKER_ID) {
      return {
        allow: false,
        reason:
          `Routing: skill "${request.callerId}" attempted direct MCP invocation; ` +
    ...
    if (request.onBehalfOf) {
      ...
      if (!skillMayCallTool(skill, request.toolName)) {
        return {
          allow: false,
          reason:
            `Routing: skill "${request.onBehalfOf}" allowlist does not include ` +
            `"${request.toolName}".`,
        };
      }
    }
  ...
  authorizeBrokerSubmission(callerId: string, toolName: string): RoutingDecision {
    ...
    if (!skillMayCallTool(skill, toolName)) {
      return {
        allow: false,
        reason:
          `Routing: skill "${callerId}" allowlist does not include "${toolName}". ` +
  ...
  authorizeCopWrite(callerId: string): RoutingDecision {
    if (callerId !== SITUATION_UNIT_ID) {
      return {
        allow: false,
        reason:
          `Routing: skill "${callerId}" attempted COP write; ` +
  ...
  authorizeCopRead(callerId: string): RoutingDecision {
  ...
  authorizeDelegation(callerId: string, targetId: string): RoutingDecision {
    const caller = this.skills.get(callerId);
    if (!caller) return { allow: false, reason: `Routing: unknown caller "${callerId}"` };
    for (const allowed of caller.permissions.invoke) {
      if (allowed === targetId) return { allow: true };
      if (allowed.endsWith("/*")) {
        const prefix = allowed.slice(0, -1);
        if (targetId.startsWith(prefix)) return { allow: true };
      }
    }
    return {
      allow: false,
      reason:
        `Routing: skill "${callerId}" cannot invoke "${targetId}". ` +
        `Allowed: [${caller.permissions.invoke.join(", ")}]`,
    };
  }
}

function skillMayCallTool(skill: SkillConfig, toolName: string): boolean {
  if (skill.mcpTools.all) return true;
