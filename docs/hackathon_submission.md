...
| 6 | Demo video (≤ 5 min, live terminal, audio narration) | [`docs/demo_video.md`](demo_video.md) - YouTube link + storyboard; [`docs/demo_video_script_v2.md`](demo_video_script_v2.md) - word-for-word narration |
...
7. **Separates evidence, deterministic comparison, and AI enrichment.** Evidence artifacts prove what happened. The Rathbun baseline gives deterministic known-good comparisons for Windows paths, hashes, services, tasks, and registry entries. Local RAG provides retrieved forensic knowledge for explaining and classifying evidence-backed behavior - MITRE technique alignment, LOLBAS context, Sigma rule rationale - instead of relying on model memory. RAG does **not** establish facts by itself. See [`docs/accuracy_report.md §3`](accuracy_report.md) for the hallucination accounting.
