...
- If any hash mismatches → Chain of custody status: **FAILED**. Halt analysis. Document the specific files that failed. Do not proceed until the discrepancy is resolved with the collecting party.
...
```bash
bash /home/sansforensics/projects/SIFTics/scripts/verify_readonly_mounts.sh /cases/<case_name>
```
...
<!-- VERIFIED = intake hashes provided and matched | GENERATED = analyst-generated, gap documented | FAILED = hash mismatch — halt -->
...
```bash
python3 /home/sansforensics/projects/SIFTics/scripts/sanitize_for_llm.py \
